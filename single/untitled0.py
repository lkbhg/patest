...
def clean_title(title: str, title_limit: int) -> str:
    ...
    while True:
        ...
        # 必须成对匹配，不能只匹配到左边
        for pattern, repl in [
            (r'^（[^（）]{1,20}）', lambda m: m.group(0)[1:-1]),  # 删除全角圆括号
            (r'^\([^()]{1,20}\)',    lambda m: m.group(0)[1:-1]),  # 英文圆括号
            (r'^【[^【】]{1,20}】',  lambda m: m.group(0)[1:-1]),  # 中文书名括号
            (r'^「[^「」]{1,20}」',  lambda m: m.group(0)[1:-1]),  # 中文引号
            (r'^『[^『』]{1,20}』',  lambda m: m.group(0)[1:-1]),  # 中文书名号
        ]:
...
def fetch_post(post_url):
    ...
    title = clean_title(raw_title, title_limit=TITLE_LIMIT)
